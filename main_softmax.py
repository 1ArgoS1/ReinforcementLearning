...
parameters = [0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1, 2, 5]


def train(temperature,arms=10, iterations=1000, tests=2000):
    # create bandit and agent.
    bandit = Bandits(arms)
    agent = Softmax(temperature,[i for i in range(arms)])
    # collect data
    data = []
...
def save_data(data,path='./data/'):
    ...
    with open(path+str(num)+"_softmax.csv","w+") as f:
        writer = csv.writer(f)
